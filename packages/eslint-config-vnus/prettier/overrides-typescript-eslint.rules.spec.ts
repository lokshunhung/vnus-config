...
describe('Config: eslint-config-prettier/@typescript-eslint', () => {
    ...
    test('only contains rules from "@typescript-eslint"', () => {
        ...
        Object.entries(presetRules).forEach((ruleEntry) => {
            ...
            expect(ruleName).toMatch(/^@typescript-eslint\/[a-z]+(-[a-z]+)*$/);
        });
    });
